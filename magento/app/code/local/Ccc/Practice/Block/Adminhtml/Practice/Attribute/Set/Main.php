...
class Ccc_Practice_Block_Adminhtml_Practice_Attribute_Set_Main extends Mage_Adminhtml_Block_Template
{
    
    protected function _construct()
    {
        $this->setTemplate('practice/attribute/set/main.phtml');
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $this->setChild('group_tree',
            $this->getLayout()->createBlock('practice/adminhtml_practice_attribute_set_main_tree_group')
        );
        ...
        $this->setChild('delete_group_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('practice')->__('Delete Selected Group'),
        ...
        $this->setChild('add_group_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('practice')->__('Add New'),
        ...
        $this->setChild('reset_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('practice')->__('Reset'),
                'onclick'   => 'window.location.reload()'
        )));

        $this->setChild('save_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('practice')->__('Save Attribute Set'),
                'onclick'   => 'editSet.save();',
                'class'     => 'save'
        )));

        $deleteConfirmMessage = $this->jsQuoteEscape(Mage::helper('practice')
        ...
        $this->setChild('delete_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('practice')->__('Delete Attribute Set'),
        ...
        $this->setChild('rename_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => Mage::helper('practice')->__('New Set Name'),
    ...
    {
        return Mage::helper('practice')->__("Edit Attribute Set '%s'", $this->_getAttributeSet()->getAttributeSetName());
    }

    
    public function getMoveUrl()
    {
        return $this->getUrl('practice/adminhtml_practice_set/save', array('id' => $this->_getSetId()));
    }

    public function getGroupUrl()
    {
        return $this->getUrl('*/practice_group/save', array('id' => $this->_getSetId()));
    }
    ...
    {    
        ...
        foreach ($groups as $node) {
            ...
            $nodeChildren = Mage::getResourceModel('practice/practice_attribute_collection')
                ->setAttributeGroupFilter($node->getId())
                ->checkConfigurableProducts()
                ->load();

                  Mage::log($nodeChildren->getData(),null,'main.log',true);
    ...
    public function getAttributeTreeJson()
    {
        ...
        if (count($items) == 0) {
            $items[] = array(
                'text'      => Mage::helper('practice')->__('Empty'),
                'id'        => 'empty',
                'cls'       => 'folder',
                'allowDrop' => false,
                'allowDrag' => false,
